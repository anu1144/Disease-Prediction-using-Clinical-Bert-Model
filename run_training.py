...
def main(flags):
    ...
    print("Trying to use %s as a pretrained BERT", flags.bert_model)
    ...
    try:
        train_dataset = read_and_preprocess_data(
            path / "C:/Users\Parul\PycharmProjects\disease-prediction\Training.csv",
            tokenizer,
            max_length=flags.seq_length
        )
        test_dataset = read_and_preprocess_data(
            path / "disease-prediction\Testing.csv",
            tokenizer,
            max_length=flags.seq_length
        )
    ...
    # if using intel, optimize the model and the optimizer
     ...
     #   import intel_extension_for_pytorch as ipex
      ...
      #      dtype = torch.bfloat16
       # else:
        #    dtype = saved_models  # default dtype for ipex.optimize()
    ...
    if flags.save_model_dir:
        ...
        print("Saved model files to %s", path)

    print("=======> Test Accuracy : %.2f",
                accuracy_score(test_preds, test_labels)
                )
    print("=======> Training Time : %.3f secs", training_time - start)
...
if __name__ == "__main__":
    ...
    if not ((pathlib.Path(FLAGS.data_dir) / "C:/Users\Parul\PycharmProjects\disease-prediction\Training.csv").exists() and
            (pathlib.Path(FLAGS.data_dir) / "C:/Users\Parul\PycharmProjects\disease-prediction\Testing.csv").exists()):
